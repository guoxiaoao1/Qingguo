...
public class TankActivity1 extends Activity {
    ...
    private LineChart linechart3=null;
    private LineChart linechart4=null;
    private List<LineChart> first=new ArrayList<>();
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_tank1);
        ...
        try {
            ...
            first.add(linechart3);
            first.add(linechart4);
            initialChart(first);
    ...
    private void addEntry(List<LineChart> mChart) {
        try {
            ...
            String jsonReader=Tools.getmethods(app.getServerAddress(),"getDieselFuelTank","");
            JSONObject json=new JSONObject(jsonReader);

            String temperature = json.optString("temperature");//温度
            String liquidLevel=json.optString("liquidLevel");//液位
            String total=json.optString("total");//累计
            String stay = json.optString("stay");//待加
            String setvalue=json.optString("setvalue");//设定
            String instant=json.optString("instant");//瞬时
            ...
            for (LineChart linechart : mChart) {
                ...
                if(linechart==linechart1) {
                    ...
                    // 增加高温
                    LineDataSet highLineDataSet = data.getDataSetByIndex(HIGH);
                    float fh = Float.parseFloat (stay);
                    Entry entryh = new Entry(fh, highLineDataSet.getEntryCount());
                    data.addEntry(entryh, HIGH);
                    ...
                    // 增加高温
                    LineDataSet highLineDataSet = data.getDataSetByIndex(HIGH);
                    float fh = Float.parseFloat (instant);
                    Entry entryh = new Entry(fh, highLineDataSet.getEntryCount());
                    data.addEntry(entryh, HIGH);
                ...
                Servera3.setText(stay+"kg");
                Servera4.setText(instant+"kg/s");
                Servera5.setText(total+"kg");
                Servera6.setText(setvalue+"kg");
    ...
    public void onClickReturnTank(View view) throws IOException {
        Intent intent = new Intent();
        intent.setClass(TankActivity1.this, SelectTankActivity.class);
